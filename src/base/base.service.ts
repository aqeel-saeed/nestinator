...
export class BaseService<T> {
    ...
    private async resolveRelations(data: DeepPartial<T> | DeepPartial<T>[]): Promise<any | any[]> {
        ...
        const resolvedItems = await Promise.all(items.map(async (item) => {
            ...
            const isSingularRelation = relation.isManyToOne || relation.isOneToOne;
            const isPluralRelation = relation.isManyToMany || relation.isOneToMany;
            ...
            const relationId = isSingularRelation
                ...
                if (isSingularRelation) {
                    if (resolvedItem[relationId]) {
                        const relatedEntity = await this.repository.manager.getRepository(relation.type).findOne({
                    ...
                    if (Array.isArray(resolvedItem[relationId])) {
                        const relatedEntities = await this.repository.manager.getRepository(relation.type).findBy({
