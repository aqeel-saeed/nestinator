...
export class PostsController extends BaseController {
  ...
  @UseAuthAndPermissionsIf(postsControllerConfig.authOptions.getUsingAuthBoolean().create)
  @Post()
  async create(@Body() post: CreatePostDto, @Req() req) {
    post['authorId'] = req.user.userId;
    const res = await this.service.create(post);
    return apiResponse(res, `${postsControllerConfig.entitySingleName} created successfully.`);
  }
}