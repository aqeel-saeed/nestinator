...
export class PermissionsGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext) {
        ...
        const config = this.reflector.get<BaseControllerConfig>('config', context.getClass());
        ...
        const permissions = this.reflector.getAllAndOverride<ControllerPermissionsOptionType>(
            'permissions',
            [
                context.getHandler(),
                context.getClass()
        ...
        const hasStandardPermission = requiredPermissions.some(
            (permission) => !permission.endsWith('_own') && userPermissions.includes(permission)
        );
